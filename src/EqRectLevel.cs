#region UICode
double Amount1 = 0; // [0,360] Zenith X
double Amount2 = 0; // [-90,90] Zenith Y
double Amount3 = 0; // [-180,180] Direction
bool Amount4 = false; // [0,1] Invert convert
...
{
    Mat3d eqRectMat
    	= Mat3d.rotY(Math.PI * -Amount3 / 180)
        * Mat3d.rotX(Math.PI * -Amount2 / 180)
        * Mat3d.rotZ(Math.PI * -Amount1 / 180);
    Mat3d persMat = Mat3d.unit;
    bool inv = Amount4;
    ...
    if (!inv) {
        ...
        dstProjection = new EqRectProjection(dst, persMat);
    } else {
        srcProjection = new EqRectProjection(src, persMat);
